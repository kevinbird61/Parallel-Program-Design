...
int main(int argc,char *argv[]){
	...
	*/
	...
	phase = 0;
	concat = (int*)malloc(size*2*sizeof(int));
	...
	for(phase=0;phase<comm_sz;phase++){
		...
		// Do Odd - and then judge
			...
			MPI_Ssend(unsorted,size,MPI_INT,my_rank-1,3,MPI_COMM_WORLD);
			...
			MPI_Ssend((concat+size),size,MPI_INT,my_rank+1,4,MPI_COMM_WORLD);
			}
			
		}
	}
	
	// Gather the result of those sorted array
	MPI_Barrier(MPI_COMM_WORLD);
	MPI_Gatherv(unsorted,size,MPI_INTEGER,sorted,rc,disp,MPI_INTEGER,0,MPI_COMM_WORLD);
	MPI_Barrier(MPI_COMM_WORLD);
	if(my_rank==0){
		printf("Sorted:\n");
		for(i=0;i<n;i++){
